...
| 3a | `step_3_train_shallow.py` | step 1 | 終端機輸出 MLP 訓練 loss 與測試準確率；`models/mlp.npz` |
| 3b | `step_3_train_cnn.py` | step 1 | 終端機輸出 CNN 訓練 loss 與測試準確率；`models/cnn.npz` |
| 4a | `step_4_inference_shallow.py` | step 3a | 終端機輸出 MLP 推理進度、10 類機率與預測結果 |
...
`step_3_train_shallow.py`：全連接層與反向傳播。`step_3_train_cnn.py`：手寫卷積、池化、im2col。兩者均不得呼叫高階深度學習 API。
...
以 [`step_4_inference_shallow.py`](step_4_inference_shallow.py) 為參考；所有 `step_N_*.py` 的終端輸出皆採相同結構。
